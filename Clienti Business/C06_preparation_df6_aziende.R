...
# - NUM_OPENs
dist_df6_num_opens_prep <- df6_az_dist_num_opens %>%
  mutate(TOT_CLIs= case_when(AVG_OPENs==1 ~ "One-time",
                                 (AVG_OPENs==2) ~ "2 times",
                                 (AVG_OPENs >2)~ "More than 2"))

dist_df6_num_opens <- as.data.frame(table(dist_df6_num_opens_prep$TOT_CLIs))


plot_dist_df6_num_opens <- ggplot(dist_df6_num_opens,aes(Var1,Freq)) +
  geom_bar(stat = "identity",fill="steelblue")+
  xlab("Number of opens") + ylab("Number of mails")
theme_minimal()

plot_dist_df6_num_opens

# - NUM_CLICKs
dist_df6_num_clicks_prep <- df6_az_dist_num_clicks %>%
  mutate(TOT_CLIs = case_when(AVG_CLICKs==1 ~ "One-time",
