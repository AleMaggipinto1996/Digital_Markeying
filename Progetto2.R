...
stato <-df_1_cli_fid %>% group_by(STATUS_FID)
stato %>% count() #si vede che la maggior parte dei clienti hanno un account attivo (***)

## START CLEANING df_1 ##
## missing values; - outliers, - dati duplicati   (***)

### Ricreare il dataset

df_1_cli_fid_clean <- df_1_cli_fid

sum(!complete.cases(df_1_cli_fid_clean)) #vediamo se ci sono casi incompleti (***)

# in questo dataset Ë inutile cercare eventuali outliers   (***)

### Check for duplicates (non duplicati per CLI-FID)

sum(duplicated(df_1_cli_fid_clean)) # vediamo se ci sono casi inseriti pi˘ volte (***)

sum(duplicated(df_1_cli_fid_clean$ID_CLI)) # ci sono 663 clienti che hanno sottoscritto pi˘ di una fidelity (***)
sum(duplicated(df_1_cli_fid_clean$ID_FID)) #non penso servi,boh (?)  (***)
...
#ci sono clienti che vedendo la data hanno probabilmente sbagliato/cambiato la fidelizzazione pi√π volte
#nello stesso giorno
...
num_fid_x_cli %>% filter (NUM_FIDs != 1) # (***)
...
df_1_cli_fid %>% filter(ID_CLI == 621814) # questo cliente specifico ad esempio inizialmente l'ha fatta online e poi in uno store dopo 1mese (?) (***)
# there could be subscriptions at the same dates [possibly for technical reasons]
df_1_cli_fid %>% filter(ID_CLI == 320880) #stesso giorno probabilmente un errore
...
##clienti business (biz)  (***)
Biz1 <- df_1_cli_fid_clean %>%
  group_by(LAST_COD_FID) %>%
  filter(LAST_COD_FID =='STANDARD BIZ')
  
biz2 <- df_1_cli_fid_clean %>%
  group_by(LAST_COD_FID) %>%
  filter(LAST_COD_FID =='PREMIUM BIZ')

df1_dist_codfid_BizTot <- merge(Biz1,biz2,all.x = TRUE,all.y = T,no.dups = T)
view(df1_dist_codfid_BizTot)

df1_dist_codfid_BIZ <- df1_dist_codfid_BizTot %>%
  group_by(LAST_COD_FID) %>%
  summarize(TOT_CLIs = n_distinct(ID_CLI)) %>%
  mutate(PERCENT = TOT_CLIs/sum(TOT_CLIs)) %>%
  arrange(desc(PERCENT))

df1_dist_codfid_BIZ  #l'81 % delle aziende ha sottoscritto un programma STANDARD

#clienti non business (***)
NoBiz1 <-df_1_cli_fid_clean %>%
  group_by(LAST_COD_FID) %>%
  filter(LAST_COD_FID =='STANDARD')

NoBiz2 <- df_1_cli_fid_clean %>%
  group_by(LAST_COD_FID) %>%
  filter(LAST_COD_FID =='PREMIUM')

df1_dist_codfid_NoBizTot <- merge(NoBiz1,NoBiz2,all.x = TRUE,all.y = T,no.dups = T)
view(df1_dist_codfid_NoBizTot)

df1_dist_codfid_NOBIZ <- df1_dist_codfid_NoBizTot %>%
  group_by(LAST_COD_FID) %>%
  summarize(TOT_CLIs = n_distinct(ID_CLI)) %>%
  mutate(PERCENT = TOT_CLIs/sum(TOT_CLIs)) %>%
  arrange(desc(PERCENT))

df1_dist_codfid_NOBIZ #l'87% delle persone fisiche hanno scelto un programma STANDARD

# le percentuali sono molto simili

## plot distribution (***)
## BIZ
plot_df1_dist_codfid_BIZ <- (
  ggplot(data=df1_dist_codfid_BIZ
         , aes(x=LAST_COD_FID, y=TOT_CLIs)
  ) +
    geom_bar(stat="identity"
             , fill="steelblue") +
...
## NO BIZ
plot_df1_dist_codfid_NOBIZ <- (
  ggplot(df1_dist_codfid_NOBIZ
         , aes(x=LAST_COD_FID, y=TOT_CLIs)
  ) +
    geom_bar(stat="identity"
             , fill="steelblue") +
...
##mantieni i valori EMAIL_PROVIDER pi√π frequenti e 
...
# troppe variabili non √® significativo
