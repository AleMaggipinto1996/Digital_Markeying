...
#Sixth STEP Train and Tesp Split.
partition_index <- createDataPartition(Churn_final$CHURN,p=0.75,list = F)
train <- Churn_final[partition_index,]
test <- Churn_final[-partition_index,]
table(train$CHURN)

ggplot(data=Churn_final, aes(x=CHURN)) +
...
N_churners <- as.numeric(table(train$CHURN)[2])
N_non_churners <- as.numeric(table(train$CHURN)[1])

Churn_0 <- filter(train,CHURN == 0)
Churn_1 <- filter(train,CHURN == 1)
Churn_1_balanced <- Churn_1[sample(N_churners,N_non_churners),]
train <- rbind(Churn_0,Churn_1_balanced)
table(train$CHURN) 
...
rpart.plot::rpart.plot(dec_tree)
